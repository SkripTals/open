...
function OpenXL(contentID) {
    const data = curcontent[contentID];
    if (!data) return;
    ...
    <div class="modal-content">
      <h2>${data.xhead}</h2>
      ${data.xcon}
      <br><button onclick="closePaymentModal()">Закрыть</button>
    </div>`;
...
function paymentFormHandle() {
    ...
    const interval = setInterval(() => {
        ...
        if (steamID && paymentLink) {
            const storeID = 15212;
            const token = '55dfd296f8326bffd31f9164332ba885';
...
curcontent["Oplata_RU"] = {
    ...
    <div class="xbox oplata_info">
      <div class="account-warning">
        ⚠️ Перед пополнением баланса проверьте, с какого аккаунта вы авторизованы!
      </div>
      <div class="forms-oplata-grid" style="display: flex; flex-direction: column; gap: 10px; margin-top: 20px;">
        <a id="qiwi-inp-other2" target="_blank" style="background: #f3f3f3; padding: 10px; border-radius: 10px; text-decoration: none; color: black; display: flex; align-items: center;">
          <img src="https://i.imgur.com/izriuxq.png" style="width: 40px; height: 40px; margin-right: 10px;">
          <div>
            <span style="font-weight: bold;">Карты (РФ)</span><br>
            <span style="font-size: 12px;">Visa, Mastercard и Мир</span>
          </div>
        </a>
        <a onclick="OpenTradeSkins()" target="_blank" style="background: #f3f3f3; padding: 10px; border-radius: 10px; text-decoration: none; color: black; display: flex; align-items: center;">
          <img src="https://i.imgur.com/k3JqyXs.png" style="width: 40px; height: 40px; margin-right: 10px;">
          <div>
            <span style="font-weight: bold;">Steam (скины)</span><br>
            <span style="font-size: 12px;">Rust и CS2</span>
          </div>
        </a>
      </div>
    </div>
